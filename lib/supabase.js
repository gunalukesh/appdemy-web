...
// ── Video upload (for content managers) ──────────────────
export const storage = {
  async uploadVideo(file, chapterId, lessonNumber) {
    const path = `chapter_${chapterId}/lesson_${lessonNumber}_${Date.now()}.mp4`
  ...
  async uploadThumbnail(file, chapterId, lessonNumber) {
    const path = `chapter_${chapterId}/thumb_${lessonNumber}_${Date.now()}.jpg`
